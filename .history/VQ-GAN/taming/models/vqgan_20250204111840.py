...
from taming.modules.diffusionmodules.model import Encoder, Decoder,cond_Decoder
...
class VQModel(pl.LightningModule):
    def __init__(self,
                 ...
                 embed_dim,
                 ckpt_path=None,
                 ignore_keys=[],#在resume时忽略的模块
                 modalities=[], # list of modalities to use #这里我们只需要用t1,t2
                 image_key="image",
    ...
    def decode(self, quant):
        quant = self.post_quant_conv(quant)#还原到量化前的维度大小
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        """optimizer_idx为1时只训练判别器(只逻辑损失),为0时训练生成器(复杂损失),lighting会在fit的时候自动按顺序调用优化器"""
        ...
        skip_pass = 1
        #stage1的时候直接自回归算损失，stage2在embedding space算损失
        if self.stage == 1: 
    ...
    def configure_optimizers(self):
        """stage1优化除了spade外其它(stage1移除了spade层),stage2优化spade"""
...
class cond_VQModel(VQModel):
    def __init__(self,
        ...
        self.image_key = image_key
        self.encoder = Encoder(**ddconfig)
        self.decoder = cond_Decoder(**ddconfig)
        self.loss = instantiate_from_config(lossconfig)
        self.quantize = VectorQuantizer(n_embed, embed_dim, beta=0.25,
                                        remap=remap, sane_index_shape=sane_index_shape)
        self.quant_conv = torch.nn.Conv3d(ddconfig["z_channels"], embed_dim, 1)
    ...
    def decode(self, quant,age):
        quant = self.post_quant_conv(quant)#还原到量化前的维度大小
        dec = self.decoder(quant,age)
        return dec
    ...
    def forward(self, input, target=None,age="unknown"):
        quant, diff, _ = self.encode(input)
        if target is not None: quant = self.spade(quant, target)
        dec = self.decode(quant,age)
        return dec, diff
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        """optimizer_idx为1时只训练判别器(只逻辑损失),为0时训练生成器(复杂损失)"""
        """source = random.choice(self.modalities)
        target = random.choice(self.modalities)"""
        source,target=random.choice(self.modalities,2)
        x_src = self.get_input(batch, source)
        x_tar = self.get_input(batch, target)
        mode=self.get_input(batch,"mode")
        age=mode if mode is not None and self.drop_rate<=random.uniform(0,1) else "unknown" 
        skip_pass = 1
        #stage1的时候直接自回归算损失，stage2在embedding space算损失
        if self.stage == 1: 
            xrec, qloss = self(x_tar,age=age)
        else:
            #decoder先不后训练，在ldm中调整
            z_src, qloss, _ = self.encode(x_src)
            z_tar_rec = self.spade(z_src, target)
            z_tar, _, _ = self.encode(x_tar)
            x_tar = z_tar
    ...
    def validation_step(self, batch, batch_idx):
        ...
        x_tar = self.get_input(batch, target)
        mode=self.get_input(batch,"mode")
        age=mode if mode is not None else "unknown" 

        if self.stage == 1: 
            xrec, qloss = self(x_tar,age)
        else:
            z_src, qloss, _ = self.encode(x_src)
            z_tar_rec = self.spade(z_src, target)
            z_tar, _, _ = self.encode(x_tar)

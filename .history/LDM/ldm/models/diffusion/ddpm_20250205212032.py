...
class LatentDiffusion(DDPM):
    ...
    def make_cond_schedule(self, ):
        """
        在num_timesteps——cond>1的时候使用
        根据推理时的可选时间步数量num_cond和时间步总数生成训练对应的时间步表
        先初始生成一个时间步长度的condition schedule,
        然后根据可选的时间步值的数量n来让时间步表的前n个满足时间步值数量对应的等间隔数列,数列后剩余的时间步表对应值仍为last time step
        如果我们在部署时始终用最后一个时间步推理就不用单独训练中间时间步了,直接设置num_timesteps_cond=1即可
        """
    ...
    def on_train_batch_start(self, batch, batch_idx, dataloader_idx):
        """如果使用scale by std,通过encoder编码第一个batch的数据的方差来设置scale为1/std"""
    ...
    def instantiate_first_stage(self, config):
        ...
        self.first_stage_model.train = disabled_train #不再训练batchnorm层，禁用dropout

...
class MyBase(Dataset):
    def __init__(self,data_paths,data_modes):
        ...
        data_dict = {path: mode for path, mode in zip(paths, modes)}
        combined_dataset=[]
        for data_path in data_dict.keys():
            data = get_my_dataset(data_path)
...
class MyTrain(MyBase):
    def __init__(self, data_path,data_modes, **kwargs):
        super().__init__(data_path=data_path,data_modes=data_modes)
